...
start(){
  ...
  xhost +local:docker
    ...
    if [ "$(docker ps -q -f name=ros2_robotica)" ]; then
        ...
        run "docker run --name ros2_robotica --shm-size=256m -v /home/agustinbravo/Desktop/robotica/taller12:/root/ros2_ws/src/robotica --env DISPLAY=$DISPLAY --volume /tmp/.X11-unix:/tmp/.X11-unix:rw --env QT_X11_NO_MITSHM=1 -it -d ros2_robotica "
    fi
}
