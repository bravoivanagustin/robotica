...
void robmovil_ekf::IMUCalibrator::calculate_bias()
{
  ...
  {
    tf2::Vector3 sum(0, 0, 0); // suma de imu omega / n
...
void robmovil_ekf::IMUCalibrator::on_imu_measurement(const sensor_msgs::msg::Imu &msg)
{
  ...
  { // si termino el tiempo de calibracion:
    ...
    delta_q.setRPY(vel_sin_bias.getX() * delta_t, vel_sin_bias.getY() * delta_t, vel_sin_bias.getZ() * delta_t);
