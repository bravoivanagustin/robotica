...
void PioneerOdometry::on_velocity_cmd(const geometry_msgs::msg::Twist::SharedPtr twist)
{
  ...
  double vLeft = (v - w * WHEEL_BASELINE / 2.0) / WHEEL_RADIUS;
  double vRight = (v + w * WHEEL_BASELINE / 2.0) / WHEEL_RADIUS;
...
void PioneerOdometry::on_encoder_ticks(const robmovil_msgs::msg::EncoderTicks::SharedPtr encoder)
{
  ...
  double delta_theta = (distance_right - distance_left) / (WHEEL_BASELINE);  // cambio de orientación
